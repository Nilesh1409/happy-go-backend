...
export const sendMobileOTP = asyncHandler(async (req, res) => {
  ...
  let entityType = "user";
  console.log("🚀 ~ sendMobileOTP ~ foundUser:", foundUser);
  if (!foundUser) {
    // If not found in User model, check Employee model
    foundEmployee = await Employee.findOne({ mobile });
    console.log("🚀 ~ sendMobileOTP ~ foundEmployee:", foundEmployee);
    if (foundEmployee) {
      entityType = "employee";
    } else {
  ...
  if (entityType === "user") {
    // For users, generate OTP via 2Factor AUTOGEN2
    const smsResult = await sendSMS({ phone: mobile });
    ...
    // For employees, no SMS sent - just set expiry time for any OTP to work
    foundEmployee.mobileVerificationOTP = "bypass"; // Placeholder value
  ...
  res.status(200).json({
    success: true,
    message: entityType === "user" ? "OTP sent successfully" : "Please enter any OTP to proceed",
  });
});
...
export const verifyMobileOTP = asyncHandler(async (req, res) => {
  ...
  // First check if this mobile belongs to an employee - if yes, bypass OTP check
  const employee = await Employee.findOne({
    mobile,
    mobileVerificationExpire: { $gt: Date.now() },
  });

  console.log("🚀 ~ verifyMobileOTP ~ employee found:", !!employee);

  if (employee) {
    // Clear OTP fields for employee
...
export const loginUser = asyncHandler(async (req, res) => {
  ...
  if (mobile) {
    ...
    if (entityType === "user") {
      ...
      console.log("🚀 ~ loginUser ~ employee bypass enabled");
    }

    return res.status(200).json({
      success: true,
      message: entityType === "user" ? "OTP sent via SMS successfully" : "Please enter any OTP to proceed",
    });
  }
